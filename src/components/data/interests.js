const interests = [
    {
        id: 1,
        name: 'Hobby Photography',
        link: 'https://www.instagram.com/developedbydrewford/',
        img: 'https://i.imgur.com/o3TGB2S.jpg',
        description: 'Learning photography with my Sony ZV1 has been a game-changer. Taking daily pictures has helped me discover hidden gems both at home and during my travels, capturing everything from small details to big moments. It’s been amazing to find beauty in the ordinary, whether I’m at home or exploring somewhere new. Each photo is a new opportunity to see the world differently.',
        linkText: 'Follow me on Instagram'
    },
    {
        id: 2,
        name: 'Local theaters',
        link: 'https://letterboxd.com/sleazycook/',
        img: 'https://i.imgur.com/ufRX8v1.jpg',
        description: 'I’ve always loved going to the movies, whether it’s solo or with my loved ones. There’s something magical about sitting in a theater, whether I’m lost in a small, grounded indie film or immersed in a big cinematic experience in Dolby Digital or IMAX. I love the feeling of being transported into another world, especially when it’s shared with others. It’s the perfect mix of personal escape and collective experience that makes the movies such a special place for me.',
        linkText: 'Follow me on Letterboxd'
    },
    {
        id: 3,
        name: 'Reading books',
        link: 'https://www.goodreads.com/user/show/144548225-andrew-cook',
        img: 'https://i.imgur.com/m5mOFW8.png',
        description: `I’m totally hooked on reading—whether it’s an addicting page-turner from Stephen King or a deeply personal nonfiction work by writers like James Baldwin, Kiese Laymon, or Bell Hooks. I love getting lost in stories that challenge and inspire, and there's nothing like the raw honesty in their words. I’m also a huge fan of comics, especially those by Brian K Vaughan or Tom King; they always pull me in with their creativity and depth. Each book, no matter the genre, feels like a new adventure.`,
        linkText: 'Follow me on Goodreads'
    },
    {
        id: 4,
        name: 'Traveling',
        link: 'https://www.instagram.com/developedbydrewford/',
        img: 'https://i.imgur.com/k4VTtn2.jpg',
        description: `Travel is one of my greatest passions, whether I’m strolling through bustling city streets or hiking through quiet nature trails. I love exploring new states, discovering hidden gems, and finding the best local restaurants along the way. Some of my favorite domestic spots are Chicago, Portland, and Los Angeles, each with its own vibe and charm. I’ve been as far as Spain, and I’m hoping to keep expanding my horizons with more international adventures in the future. There's always something new to discover and I can't wait to see where my travels take me next.`,
        linkText: 'Follow me on Instagram'
    },
    {
        id: 5,
        name: 'Concerts & festivals',
        link: 'https://www.instagram.com/developedbydrewford/',
        img: 'https://i.imgur.com/Mgml5mj.png',
        description: `I’m all about live music—there’s something special about the energy of concerts and festivals that you just can’t get anywhere else. Whether it’s the electrifying atmosphere or the sheer spectacle of the performers, I love getting lost in the moment. I try to hit up shows at least once a month and make it to a festival every year. Some of my favorite performances have been by Tyler the Creator, Paul McCartney, SiR, and Ari Lennox—each one unforgettable in its own way. There’s nothing like that shared excitement and connection with the crowd.`,
        linkText: 'Follow me on Instagram'
    },
    {
        id: 6,
        name: `Women's Basketball`,
        link: 'https://www.wnba.com',
        img: 'https://i.imgur.com/egaLscf.png',
        description: `I didn’t really get into sports until my 30s, but watching Angel Reese’s rookie year in the WNBA completely hooked me. The culture around women’s sports feels so much more inclusive and wholesome compared to the often intense vibe in mainstream sports. I love traveling to different cities to catch teams play in their hometowns, but when I’m not on the road, I try to catch every game from home. There’s something special about the energy and the community in women’s sports that makes it all feel more personal and exciting.`,
        linkText: 'See more at WNBA'
    },
    {
        id: 7,
        name: 'Video Games',
        link: 'https://steamcommunity.com/profiles/76561198829311676/',
        img: 'https://i.imgur.com/tRrzASy.png',
        description: `Video games have been a part of my life for as long as I can remember. Some of my best memories are from sitting on the couch with friends and family, playing Nintendo and just having a blast together. As I got older, I started gravitating more towards narrative-driven single-player games that tell amazing stories, but I also love diving into online multiplayer experiences where I can team up with friends across the country. Gaming has always been a way to connect, unwind, and get lost in different worlds.`,
        linkText: 'Follow me on Steam'
    },
    {
        id: 8,
        name: 'Board & Card Games',
        link: 'https://boardgamegeek.com/boardgame/10547/betrayal-at-house-on-the-hill',
        img: 'https://i.imgur.com/C7HKW3p.png',
        description: `I’ve always loved board games and card games—the joy of gathering around a table with others and getting lost in the competition and camaraderie is something truly special. Whether it’s a classic game of Spades, a quick round of Uno, the strategy of Catan, or even something more immersive like Dungeons & Dragons, the in-person experience of playing together is what makes it so magical. There’s just something about that shared energy and laughter that makes every game memorable.`,
        linkText: 'Learn about Betrayal'
    },

]

export default interests;